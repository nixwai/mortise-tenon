...
function resizeHorizontal(resizeData: ResizeData, resizingWidthFn: ResizingFn) {
  ...
  const { width: domWidth, matrix } = resizeData.domAttrs;
  const dir = resizingWidthFn === resizeData.resizingForward ? 1 : -1;
  const { value: width, offset: translateX } = resizingWidthFn(domWidth, domWidth + dir * distanceX, 'x');
  resizeData.setStyleWidthOrHeight(width, 'width');
  resizeData.setStyletTransform(translateX, matrix.translateY);
}

/** 调整垂直方向 */
function resizeVertical(resizeData: ResizeData, resizingHeightFn: ResizingFn) {
  const { distanceY = 0 } = resizeData.options;
  const { height: domHeight, matrix } = resizeData.domAttrs;
  const dir = resizingHeightFn === resizeData.resizingForward ? 1 : -1;
  const { value: height, offset: translateY } = resizingHeightFn(domHeight, domHeight + dir * distanceY, 'y');
  resizeData.setStyleWidthOrHeight(height, 'height');
  resizeData.setStyletTransform(matrix.translateX, translateY);
}
...
function resizeHorizontalAndVertical(resizeData: ResizeData, resizingWidthFn: ResizingFn, resizingHeightFn: ResizingFn) {
  ...
  const updateDom = (options: { distanceX: number, distanceY: number }) => {
    ...
    const { value: width, offset: translateX } = resizingWidthFn(domWidth, domWidth + dirX * options.distanceX, 'x');
    const { value: height, offset: translateY } = resizingHeightFn(domHeight, domHeight + dirY * options.distanceY, 'y');
    resizeData.setStyleWidthOrHeight(width, 'width');
    resizeData.setStyleWidthOrHeight(height, 'height');
